...
#define GLOBAL_OFF(sensor, bin, channel, bin_size, num_sensors_per_block, num_bins, first_sensor_in_block) \
  (first_sensor_in_block) * (num_bins) * (bin_size) + SHARED_OFF(sensor, bin, channel, bin_size, num_sensors_per_block)

#define GLOBAL_OFF_B(sensor, bin, num_sensors_per_block, num_bins, first_sensor_in_block) \
  (first_sensor_in_block) * (num_bins) + SHARED_OFF_B(sensor, bin, num_sensors_per_block)
...
template <typename T>
__global__ void
MultiTau::correlate<T>(T *new_values, const int timepoints, int instants_processed, T *shift_register, uint16_t *shift_positions, T *accumulators, T *correlation, const int num_bins, const int num_sensors) {
  int num_sensors_per_block = blockDim.y;
  int bin_size = blockDim.x;

  int local_sensor_id = threadIdx.y;  // relative sensor id inside the block -> 0..num_sensors_per_block-1
  int channel = threadIdx.x;          // channels goes from 0..bin_size-1

  int first_sensor_in_block = blockIdx.x * num_sensors_per_block;  // id for the first sensor of the block
  int global_sensor_id = first_sensor_in_block + local_sensor_id;  // global sensor id -> 0..num_sensors-1
  ...
  uint16_t *block_shift_pos = reinterpret_cast<uint16_t *>(&block_accumulators[num_sensors_per_block * num_bins]);

  int clamp_mask = first_sensor_in_block + num_sensors_per_block >= num_sensors;
  num_sensors_per_block = (num_sensors - first_sensor_in_block) * clamp_mask + num_sensors_per_block * (1 - clamp_mask);

  if(global_sensor_id < num_sensors) {
    // Copy correlator arrays from global memory to shared memory
    for(int bin = 0; bin < num_bins; ++bin) {
      block_shift[SHARED_OFF(local_sensor_id, bin, channel, bin_size, num_sensors_per_block)] = shift_register[GLOBAL_OFF(local_sensor_id, bin, channel, bin_size, num_sensors_per_block, num_bins, first_sensor_in_block)];
      block_correlation[SHARED_OFF(local_sensor_id, bin, channel, bin_size, num_sensors_per_block)] = correlation[GLOBAL_OFF(local_sensor_id, bin, channel, bin_size, num_sensors_per_block, num_bins, first_sensor_in_block)];

      block_accumulators[SHARED_OFF_B(local_sensor_id, bin, num_sensors_per_block)] = accumulators[GLOBAL_OFF_B(local_sensor_id, bin, num_sensors_per_block, num_bins, first_sensor_in_block)];
      block_shift_pos[SHARED_OFF_B(local_sensor_id, bin, num_sensors_per_block)] = shift_positions[GLOBAL_OFF_B(local_sensor_id, bin, num_sensors_per_block, num_bins, first_sensor_in_block)];
    }
    ...
    for(int instant = 0; instant < timepoints; ++instant) {
      ...
      T new_value = new_values[instant * num_sensors + global_sensor_id];

      int insert_channel_fb = block_shift_pos[SHARED_OFF_B(local_sensor_id, 0, num_sensors_per_block)];  // fb stands for first bin

      if(channel == 0) {  // only one thread add the new_value to the sensor shift register
        block_shift[SHARED_OFF(local_sensor_id, 0, insert_channel_fb, bin_size, num_sensors_per_block)] = new_value;
        block_accumulators[SHARED_OFF_B(local_sensor_id, 0, num_sensors_per_block)] += new_value;
      }

      __syncthreads();

      block_correlation[SHARED_OFF(local_sensor_id, 0, channel, bin_size, num_sensors_per_block)] += block_shift[SHARED_OFF(local_sensor_id, 0, insert_channel_fb, bin_size, num_sensors_per_block)] * block_shift[SHARED_OFF(local_sensor_id, 0, (insert_channel_fb - channel + bin_size) & (bin_size - 1), bin_size, num_sensors_per_block)];
      block_shift_pos[SHARED_OFF_B(local_sensor_id, 0, num_sensors_per_block)] = (insert_channel_fb + 1) & (bin_size - 1);  // x % y == x & (y-1)

      size_t max_bin = insert_until_bin(instants_processed, num_bins);

      for(int bin = 1; bin < max_bin; ++bin) {
        int insert_channel = block_shift_pos[SHARED_OFF_B(local_sensor_id, bin, num_sensors_per_block)];

        if(channel == 0) {  // only one thread add the new_value to the sensor shift register
          block_shift[SHARED_OFF(local_sensor_id, bin, insert_channel, bin_size, num_sensors_per_block)] = block_accumulators[SHARED_OFF_B(local_sensor_id, bin - 1, num_sensors_per_block)];
          block_accumulators[SHARED_OFF_B(local_sensor_id, bin, num_sensors_per_block)] += block_accumulators[SHARED_OFF_B(local_sensor_id, bin - 1, num_sensors_per_block)];
        }

        __syncthreads();

        block_accumulators[SHARED_OFF_B(local_sensor_id, bin - 1, num_sensors_per_block)] = 0;

        block_correlation[SHARED_OFF(local_sensor_id, bin, channel, bin_size, num_sensors_per_block)] += block_shift[SHARED_OFF(local_sensor_id, bin, insert_channel, bin_size, num_sensors_per_block)] * block_shift[SHARED_OFF(local_sensor_id, bin, (insert_channel - channel + bin_size) & (bin_size - 1), bin_size, num_sensors_per_block)];
        block_shift_pos[SHARED_OFF_B(local_sensor_id, bin, num_sensors_per_block)] = (insert_channel + 1) & (bin_size - 1);  // x % y == x & (y-1)
      }
    }

    // Copy correlator arrays from shared memory to global memory
    for(int bin = 0; bin < num_bins; ++bin) {
      shift_register[GLOBAL_OFF(local_sensor_id, bin, channel, bin_size, num_sensors_per_block, num_bins, first_sensor_in_block)] = block_shift[SHARED_OFF(local_sensor_id, bin, channel, bin_size, num_sensors_per_block)];
      correlation[GLOBAL_OFF(local_sensor_id, bin, channel, bin_size, num_sensors_per_block, num_bins, first_sensor_in_block)] = block_correlation[SHARED_OFF(local_sensor_id, bin, channel, bin_size, num_sensors_per_block)];

      accumulators[GLOBAL_OFF_B(local_sensor_id, bin, num_sensors_per_block, num_bins, first_sensor_in_block)] = block_accumulators[SHARED_OFF_B(local_sensor_id, bin, num_sensors_per_block)];
      shift_positions[GLOBAL_OFF_B(local_sensor_id, bin, num_sensors_per_block, num_bins, first_sensor_in_block)] = block_shift_pos[SHARED_OFF_B(local_sensor_id, bin, num_sensors_per_block)];
    }
  }
};
...
Correlator<T>::Correlator(const int t_num_bins, const int t_bin_size, const int t_num_sensors, const int t_packet_size, const int t_device, const bool t_debug) {
  ...
  int shift_registers_pos_block_usage = num_bins * sizeof(uint16_t);
...
void Correlator<T>::alloc() {
  ...
  CHECK(cudaMalloc(&d_shift_positions, num_bins * num_sensors * sizeof(uint16_t)));
  ...
  CHECK(cudaMemset(d_shift_positions, 0, num_bins * num_sensors * sizeof(uint16_t)));
  ...
  if(debug) std::cout << "[INFO] Allocating host arrays" << std::endl;

  correlation = (T *)malloc(num_bins * bin_size * num_sensors * sizeof(T));
};

template <typename T>
void Correlator<T>::correlate(const T *new_values, const int num_values) {
  transferred = false;

  if(debug) std::cout << "[INFO] Allocating and copying new values to gpu array" << std::endl;

  CHECK(cudaMemcpy(d_new_values, new_values, num_values * num_sensors * sizeof(T), cudaMemcpyHostToDevice));

  if(debug) std::cout << "[INFO] Starting correlation" << std::endl;

  MultiTau::correlate<T><<<number_of_blocks, threads_per_block, shared_memory_per_block>>>(d_new_values, num_values, instants_processed, d_shift_register, d_shift_positions, d_accumulators, d_correlation, num_bins, num_sensors);
  cudaDeviceSynchronize();
  CHECK_KERNELCALL();

  instants_processed += num_values;
...
void Correlator<T>::transfer() {
  ...
  if(debug) std::cout << "[INFO] Data transferred" << std::endl;
}

template <typename T>
T Correlator<T>::get(const int sensor, const int lag) {
  assert(transferred && "ERROR: Data not transfered from device memory to host memory");
